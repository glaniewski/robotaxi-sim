"""
Experiment 53 — Same as Exp52, but fleet starts at SOC 50% (`soc_initial=0.5`).

`soc_charge_start` / `soc_target` remain 0.80 (same charging goals as Exp52).
Compare EOD fleet SOC and service vs Exp52 (80% start). Requires OSRM + parquet data.
"""
...
CHARGER_KW = 20.0
SITE_POWER_KW = 80.0
# Only change vs Exp52: cold start
SOC_INITIAL = 0.5
...
def run_one(
    ...
    desc = f"exp53 soc50 {depot_selection}"
    bar = tqdm(
        total=total_reqs,
        desc=desc,
        unit="trips",
...
def main() -> None:
    ...
    rows: list[dict] = []
    for sel in tqdm(RUNS, desc="exp53 soc50 depot selection", unit="run"):
