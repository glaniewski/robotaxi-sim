"""
Experiment 54 — Iteration 1: mandatory soc_min floor + supply-aware charging sweep

Baseline: Exp52 fastest config (coverage_floor, fleet=3000, scale=0.1, max_wait=600s,
50 microsites, 4x20kW, JIT, soc_charge_start=0.80, soc_target=0.80, soc_initial=0.80).

Sweep: charge_supply_ratio × max_concurrent_charging_pct.
Run 0: supply-aware disabled (ratio=9999, cap=1.0) — tests mandatory floor only.
Run 1-4: supply-aware at various caps.
"""
...
SWEEP = [
    {"label": "floor_only",     "charge_supply_ratio": 9999.0, "max_concurrent_charging_pct": 1.0},
    {"label": "sa_cap10",       "charge_supply_ratio": 2.0,    "max_concurrent_charging_pct": 0.10},
    {"label": "sa_cap15",       "charge_supply_ratio": 2.0,    "max_concurrent_charging_pct": 0.15},
    {"label": "sa_cap20",       "charge_supply_ratio": 2.0,    "max_concurrent_charging_pct": 0.20},
    {"label": "sa_cap25",       "charge_supply_ratio": 2.0,    "max_concurrent_charging_pct": 0.25},
    {"label": "sa_ratio1.5_c15","charge_supply_ratio": 1.5,    "max_concurrent_charging_pct": 0.15},
]
...
def run_one(
    ...
    sc = SimConfig(
        ...
        charge_supply_ratio=cfg["charge_supply_ratio"],
        max_concurrent_charging_pct=cfg["max_concurrent_charging_pct"],
    )
    ...
    soc_initial = sc.soc_initial
    net_energy_pct = round(m["fleet_battery_pct"] - soc_initial * 100, 2)
    return {
        "label": label,
        "charge_supply_ratio": cfg["charge_supply_ratio"],
        "max_concurrent_charging_pct": cfg["max_concurrent_charging_pct"],
...
def main() -> None:
    ...
    rows: list[dict] = []
    for cfg in tqdm(SWEEP, desc="exp54 sweep", unit="run"):
