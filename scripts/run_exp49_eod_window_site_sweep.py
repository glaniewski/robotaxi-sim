...
# Historical name: previously measured post-horizon EOD drain; engine now stops at horizon only.
MICROSITE_COUNTS = [200, 400, 600]
PLUGS_PER_SITE = 4
CHARGER_KW = 20.0
SITE_POWER_KW = 80.0
...
def run_one(n_sites: int, dcw: dict[str, int], dcs: set[str], covered_by: dict) -> dict:
    ...
    bar = tqdm(
        total=total_reqs,
        desc=f"exp49 microsites={n_sites}",
        unit="trips",
        ncols=100,
    ...
    below_target = m["vehicles_below_soc_target_count"]
    min_final_soc = min(v.soc for v in vlist) if vlist else 0.0
    cc_times = [t for t, et in res.get("event_log", []) if et == EventType.CHARGING_COMPLETE.value]
    last_cc = max(cc_times) if cc_times else None
    ...
    return {
        ...
        "below_soc_target_count": below_target,
        "min_final_soc": round(min_final_soc, 6),
        "last_charging_complete_min": round(last_cc / 60.0, 3) if last_cc is not None else None,
        "wall_s": wall,
    }
...
def main() -> None:
    ...
    rows: list[dict] = []
    for n in tqdm(MICROSITE_COUNTS, desc="exp49 sweep", unit="config"):
