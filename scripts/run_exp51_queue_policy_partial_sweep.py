"""
Experiment 51 — Partial sweep: JIT vs FIFO depot charging queue (microsites)

Pairs Exp44-style baseline per-site hardware (4 plugs × 20 kW, 80 kW site) with
`charging_queue_policy` jit vs fifo at two site counts (50, 80).

Partial sweep: 4 runs total. Requires OSRM (e.g. localhost:5001).
"""
...
def run_one(
    ...
    sc = SimConfig(
        ...
        kwh_per_mile=0.20,
        charging_queue_policy=queue_policy,
    )
    ...
    desc = f"exp51 sites={n_sites} {queue_policy}"
    bar = tqdm(
        total=total_reqs,
        desc=desc,
        unit="trips",
...
def main() -> None:
    ...
    rows: list[dict] = []
    for n_sites, pol in tqdm(RUNS, desc="exp51 sweep", unit="run"):
        row = run_one(n_sites, pol, dcw, dcs, covered_by)
        rows.append(row)
