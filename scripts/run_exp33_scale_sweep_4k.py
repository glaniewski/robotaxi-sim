"""
Experiment 33 — 1×, 2×, 3× scale sweep at fleet=4k
==================================================
Runs scale=0.1 (1×), 0.2 (2×), 0.3 (3×) with fleet=4,000. Same policy as Exp 30
(cov_floor+optB, travel=60m, look=60m, floor=2). Tests service consistency across
demand multiples.

Usage:
    PYTHONHASHSEED=0 python scripts/run_exp33_scale_sweep_4k.py
"""
...
# 1×, 2×, 3× relative to Exp 30 (scale 0.1)
SCALES = (0.1, 0.2, 0.3)
FLEET_SIZE = 4000
...
def main() -> None:
    ...
    SEP = "-" * 150
    print(f"Exp 33 — 1×, 2×, 3× scale sweep at fleet={FLEET_SIZE:,}  (max_wait={int(MAX_WAIT)}s)\n")
    print(HDR)
    print(SEP)
...
Scale=0.1 (1×), 0.2 (2×), 0.3 (3×) with fleet=4,000; same policy as Exp 30 (cov_floor+optB, travel=60m, look=60m, floor=2).  
Config: `seed=123`, `duration=1440min`, `max_wait=600s`. depots=[].

| scale | fleet | policy | trips | served% | expired | p50 | p90 | p99 | move mi/v | revenue% (dead%) | t/veh | REPO | wall |
|-------|-------|--------|-------|---------|---------|-----|-----|-----|-----------|------------------|-------|------|------|
{md_rows}
