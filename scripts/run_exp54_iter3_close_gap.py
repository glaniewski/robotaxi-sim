"""
Experiment 54 iter3 — Close the net-energy gap.

Hold 50 sites × 8 plugs × 50kW (the best infra from iter2: net_energy = -0.34%).
Sweep max_concurrent_charging_pct from 10% to 14% to find the point where net_energy >= 0.
"""
...
SWEEP = [
    {"label": "cap10", "charge_supply_ratio": 2.0, "max_concurrent_charging_pct": 0.10},
    {"label": "cap11", "charge_supply_ratio": 2.0, "max_concurrent_charging_pct": 0.11},
    {"label": "cap12", "charge_supply_ratio": 2.0, "max_concurrent_charging_pct": 0.12},
    {"label": "cap13", "charge_supply_ratio": 2.0, "max_concurrent_charging_pct": 0.13},
    {"label": "cap14", "charge_supply_ratio": 2.0, "max_concurrent_charging_pct": 0.14},
]
...
def run_one(cfg: dict, dcw: dict[str, int], dcs: set[str], covered_by: dict) -> dict:
    ...
    depots = [
        Depot(
            ...
            chargers_count=PLUGS_PER_SITE, charger_kw=CHARGER_KW,
    ...
    label = cfg["label"]
    bar = tqdm(
        total=total_reqs, desc=f"exp54i3 {label}", unit="trips", ncols=110,
...
def main() -> None:
    ...
    rows: list[dict] = []
    for cfg in tqdm(SWEEP, desc="exp54i3 sweep", unit="run"):
