"""
Experiment 50 — Per-site plug count & kW A/B (80 microsites, no EOD tail)

Compares Exp44-style stack at fixed 80 demand-origin microsites:
- Baseline: 4 plugs × 20 kW, 80 kW site power (shared cap).
- Variant:  8 plugs × 50 kW, 400 kW site power.

Engine stops at horizon only (no post-horizon charging sweep).
"""
...
RUNS = [
    ("baseline_4x20_80kw", 4, 20.0, 80.0),
    ("high_8x50_400kw", 8, 50.0, 400.0),
]
...
def run_one(
    ...
    return {
        ...
        "charger_kw": charger_kw,
        "site_power_kw": site_power_kw,
        "plugs_total": N_SITES * plugs,
        "nameplate_kw": N_SITES * site_power_kw,
        ...
        **depot_u,
        ...
        "chg_time_fleet_min": round(chg_time_min, 1),
        "chg_time_per_vehicle_min": round(chg_time_min / FLEET, 3) if FLEET else 0.0,
        "wall_s": round(wall, 1),
    }
...
def main() -> None:
    ...
    rows: list[dict] = []
    for label, plugs, ckw, skw in tqdm(RUNS, desc="exp50 A/B", unit="run"):
        row = run_one(label, plugs, ckw, skw, dcw, dcs, covered_by)
        rows.append(row)
