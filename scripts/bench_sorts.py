"""
Run Exp 27 sim N times (no diagnostics). Prints wall_s and unserved_count per run.
Use for: (1) with sorts, (2) without sorts + PYTHONHASHSEED=0.
"""
...
def run_once() -> tuple[float, int]:
    ...
    policy = build_policy(
        ...
        coverage_reposition_travel_minutes=30.0, timed_forecast_table=timed,
        forecast_bucket_minutes=BUCKET_MIN, coverage_lookahead_minutes=30.0,
    )
    ...
    unserved = sum(1 for r in eng.requests.values() if r.status == RequestStatus.UNSERVED)
    return wall, unserved


def main() -> None:
    n = int(sys.argv[1]) if len(sys.argv) > 1 else 3
    print(f"Exp 27 config, no diagnostics. Running {n} times...")
    for i in range(n):
        wall, unserved = run_once()
        print(f"  run {i+1}: wall={wall:.2f}s  unserved={unserved}")
