"""
One-off: Exp 27 replication (coverage_floor+optB floor=2, scale=0.01, fleet=3000)
to verify drain fix: no PENDING at sim end.
"""
...
def main() -> None:
    ...
    served = by_status.get(RequestStatus.SERVED, 0)
    unserved = by_status.get(RequestStatus.UNSERVED, 0)
    pending = by_status.get(RequestStatus.PENDING, 0)
    total = len(eng.requests)
    print("Exp 27 replication (floor=2, scale=0.01, fleet=3000)")
    print(f"  SERVED: {served}  UNSERVED: {unserved}  PENDING: {pending}  total: {total}")
    print(f"  served_pct: {100 * served / total:.2f}%  wall: {wall:.1f}s")
    if pending == 0:
        print("  -> Drain fix OK: 0 PENDING at sim end.")
    else:
        print("  -> Issue: still have PENDING at sim end.")
