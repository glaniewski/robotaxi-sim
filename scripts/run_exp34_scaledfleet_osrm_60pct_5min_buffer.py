"""
Experiment — OSRM time multiplier + pickup/dropoff time buffer
==============================================================
...
This experiment is intended to apply:
  - OSRM_TIME_MULTIPLIER=1.6  (OSRM travel-time inflation)
  - OSRM_PICKUP_DROPOFF_BUFFER_MINUTES=5 (rider boarding/alighting buffer)

The simulation logic reads these from environment variables (see RoutingCache
and SimConfig defaults). This script simply runs the three scenarios and
appends a table to RESULTS.md.
"""
...
SCALE_FLEET = [(0.1, 4000), (0.2, 8000), (0.3, 12000)]
...
def run_one(scale: float, fleet_size: int, dcw, dcs, covered_by, routing) -> dict:
    ...
    # Same policy family/parameters as Exp 30/32.
    policy = build_policy(
        name="coverage_floor",
        alpha=0.6,
...
def main() -> None:
    ...
    run_label = (
        "Experiment — OSRM x1.6 + pickup/dropoff buffer=5m, scaled fleet "
        "(scale 0.1@4k, 0.2@8k, 0.3@12k)"
    )
...
## Experiment 34 — OSRM time inflation + pickup/drop buffer (scaled fleet)
