"""
Experiment 37 — Find fleet for served_pct >= 99% (scale=0.1)
=============================================================

Goal: identify the smallest fleet size (in a tested grid) such that
`served_pct >= 99%` at demand `scale=0.1`.

This script:
  - runs `scale=0.1` with increasing `fleet_size`
  - stops once the threshold is met
  - appends a markdown table to `RESULTS.md`

Policy family/params: same as Exp 30/32/34/36 (coverage_floor + cov_floor+optB).
"""
...
# Increasing grid; script stops at first fleet with served_pct >= 99.
FLEET_SIZES = [4000, 4250, 4500, 4750, 5000, 5250]

SERVE_THRESHOLD_PCT = 99.0
...
def run_one(
    ...
    # util% = time spent moving throughout the entire day.
...
def main() -> None:
    # Ensure deterministic OSRM timing/buffer settings (defaults for "no extra inflation").
    os.environ["OSRM_TIME_MULTIPLIER"] = "1.0"
    os.environ["OSRM_PICKUP_DROPOFF_BUFFER_MINUTES"] = "0.0"
    ...
    for fleet_size in FLEET_SIZES:
        ...
        rows.append(r)
        print(
            f"  fleet={fleet_size:,} served_pct={r['served_pct']:.3f}% "
            f"expired={r['expired']:,} p50={r['p50']:.2f}m wall={r['wall']:.1f}s"
        )
        if r["served_pct"] >= SERVE_THRESHOLD_PCT:
            print(
                f"Threshold met: fleet={fleet_size:,} served_pct={r['served_pct']:.3f}% "
                f">= {SERVE_THRESHOLD_PCT:.1f}%"
            )
            break
...
## Experiment 37 — scale=0.1 fleet sweep to reach served_pct >= {SERVE_THRESHOLD_PCT:.1f}%

Config: `seed=123`, `duration=1440min`, `max_wait=600s`, `reposition=cov_floor+optB` (same as Exp 30/32/34/36). depots=[].
Environment:
