"""
Experiment 33 — add scale=0.2 @ 8k and scale=0.3 @ 12k
======================================================
Runs (0.2, 8000) and (0.3, 12000); inserts rows into the existing Exp 33 table
so 0.2 has 4k and 8k adjacent, 0.3 has 4k and 12k adjacent. Same policy as Exp 30.

Usage:
    PYTHONHASHSEED=0 python3 scripts/run_exp33_8k_12k.py
"""
...
def insert_rows_into_exp33(rows_by_scale: dict) -> None:
    """Insert 0.2@8k after 0.2@4k row, 0.3@12k after 0.3@4k row in Experiment 33."""
    results_path = ROOT / "RESULTS.md"
    content = results_path.read_text()
    exp32_end = content.find("\n## Experiment 33 —")
    if exp32_end == -1:
        exp33_start = content.find("## Experiment 33 —")
    else:
        exp33_start = exp32_end + 1
    if exp33_start == -1:
        print("Experiment 33 section not found in RESULTS.md; skipping insert.")
        return

    # Insert 0.3× 12k first (after 0.3× 4k), then 0.2× 8k (after 0.2× 4k), so positions don't shift
    if (0.3, 12000) in rows_by_scale:
        r = rows_by_scale[(0.3, 12000)]
        new_line = row_to_md(r) + "\n"
        marker = "| 0.3× | 4,000 | cov_floor+optB |"
        pos = content.find(marker, exp33_start)
        if pos != -1:
            line_end = content.find("\n", pos)
            content = content[: line_end + 1] + new_line + content[line_end + 1 :]
    # 0.2× 8k after "| 0.2× | 4,000 |"
    if (0.2, 8000) in rows_by_scale:
        r = rows_by_scale[(0.2, 8000)]
        new_line = row_to_md(r) + "\n"
        marker = "| 0.2× | 4,000 | cov_floor+optB |"
        pos = content.find(marker, exp33_start)
        if pos != -1:
            line_end = content.find("\n", pos)
            content = content[: line_end + 1] + new_line + content[line_end + 1 :]
    ...
    print(f"\nInserted 0.2@8k and 0.3@12k rows into Experiment 33 in {results_path}")
...
def main() -> None:
    ...
    SEP = "-" * 150
    print("Exp 33 — scale=0.2 @ 8k, scale=0.3 @ 12k  (max_wait=600s)\n")
    print(HDR)
    print(SEP)

    rows_by_scale: dict[tuple[float, int], dict] = {}
    for scale, fleet in RUNS:
        r = run_one(scale, fleet, dcw, dcs, covered_by, routing)
        rows_by_scale[(scale, fleet)] = r
        print(
    ...
    print(f"\n{SEP}\nDone.")
    insert_rows_into_exp33(rows_by_scale)
