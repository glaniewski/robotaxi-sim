"""
Experiment 31 — Full demand scale=1.0, fleet=30k (engine perf baseline)
======================================================================
First run with all trips (no demand scaling) to gauge current engine performance.
Same policy as Exp 30: cov_floor+optB, travel=60m, look=60m, floor=2.

Usage:
    PYTHONHASHSEED=0 python scripts/run_exp31_full_scale.py
"""
...
def main() -> None:
    ...
    SEP = "-" * 150
    print(f"Exp 31 — cov_floor+optB scale={SCALE} fleet={FLEET_SIZE:,}  (max_wait={int(MAX_WAIT)}s)\n")
    print(HDR)
    print(SEP)
    ...
    wall_hr = r["wall"] / 3600.0
    wall_str = f"~{wall_hr:.1f} h" if wall_hr >= 1 else f"{r['wall'] / 60:.0f}m"
    md_block = f"""
---

## Experiment 31 — Full demand scale=1.0, fleet=30k (engine perf baseline)

First run with all trips (demand_scale=1.0) to gauge current engine performance.  
Config: `seed=123`, `duration=1440min`, `max_wait=600s`, `travel=60m`, `look=60m`, `floor=2`, cov_floor+optB.  
**Note:** depots=[] so charging metrics are 0.

| fleet | policy | trips | served% | expired | p50 | p90 | p99 | move mi/v | revenue% (dead%) | time move% (dead%) | t/veh | REPO | wall |
|-------|--------|-------|---------|---------|-----|-----|-----|-----------|------------------|--------------------|-------|------|------|
| {r['fleet']:,} | cov_floor+optB | {r['trips']:,} | {r['served_pct']:.3f}% | {r['expired']:,} | {r['p50']:.1f}m | {r['p90']:.1f}m | {r['p99']:.1f}m | {r['move_mi_veh']:.1f} | {r['revenue_mi_pct']:.1f}% ({r['deadhead_pct']:.1f}%) | {r['move_time_pct']:.1f}% ({r['deadhead_time_pct']:.1f}%) | {r['trips_per_veh']:.1f} | {r['repo']:,} | {wall_str} |
