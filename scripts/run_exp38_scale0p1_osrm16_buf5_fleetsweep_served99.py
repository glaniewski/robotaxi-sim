"""
Experiment 38 — scale=0.1 fleet sweep (OSRM x1.6, buffer=5m) to reach served_pct >= 99%
======================================================================================

Goal: smallest fleet size (in a tested 1k grid) such that
`served_pct >= 99%` at demand `scale=0.1`, when:
  - `OSRM_TIME_MULTIPLIER=1.6`
  - `OSRM_PICKUP_DROPOFF_BUFFER_MINUTES=5`

Stops once the threshold is met and appends a markdown table to RESULTS.md.
"""
...
# Start at 6k and increment by 1k as requested.
FLEET_SIZES = list(range(6000, 20001, 1000))
...
## Experiment 38 — scale=0.1 OSRM x{osrm_time_mult_val:g} + buffer={pickup_dropoff_buf_min_val:g}m fleet sweep to reach served_pct >= {SERVE_THRESHOLD_PCT:.1f}%

Config: `seed=123`, `duration=1440min`, `max_wait=600s`, `reposition=cov_floor+optB` (same as Exp 30/32/34/36/37). depots=[].
Environment:
